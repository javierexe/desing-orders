...
const AmountDetection = ({ 
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      // Usar parser frontend para detectar montos
      const amountResult = detectMostLikelyAmount(ocrResult.text);
      console.log('🔄 AmountDetection: Frontend detection result:', amountResult);
      
      setDetectionResult({
        amount: amountResult.amount,
        confidence: amountResult.confidence,
        allMatches: amountResult.allMatches,
   ...
   */
  const rejectAmount = () => {
    // Cargar archivo sin monto automático, dejar entrada manual
    if (onAmountDetected) {
      onAmountDetected(null, selectedItemId, 'manual'); // Indicar modo manual
    }
    // Llamar onCancel para cerrar componente y mostrar entrada manual
    onCancel();
  };
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 mt-4">
      ...
      {detectionResult && !error && (
        <div className="space-y-4">
          <div className={`${getConfidenceStyles(detectionResult.confidence).bg} ${getConfidenceStyles(detectionResult.confidence).border} border rounded-lg p-4`}>
            <div className="flex items-center space-x-2 mb-3">
              <CheckCircle className={`w-5 h-5 ${getConfidenceStyles(detectionResult.confidence).icon}`} />
              <h4 className={`${getConfidenceStyles(detectionResult.confidence).titleText} font-medium`}>Monto detectado</h4>
            </div>
            
            <div className="text-center mb-4">
              <div className={`text-3xl font-bold ${getConfidenceStyles(detectionResult.confidence).amountText} mb-1`}>
                {formatChileanAmount(detectionResult.amount)}
              </div>
              <div className={`text-sm ${getConfidenceStyles(detectionResult.confidence).text}`}>
